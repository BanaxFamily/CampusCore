...
import DeanForApproval from "../dean/courses/submission/DeanForApproval";
import Approved from "../dean/courses/submission/Approved";

export default function PrcApprovedSubmissions() {
  ...
  useEffect(() => {
    async function getAllApprovedSubmissions() {
      try {
        const response = await SubmissionApi.getAllSubmissionPrc({ "isApproved": true })
    ...
    async function getAllForApprovalSubmissions() {
      try {
        const response = await SubmissionApi.getAllSubmissionPrc({ "isApproved": false })
        if (response.isSuccess) {
