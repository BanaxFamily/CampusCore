                    ...
                    facultyAsideLinks.map((nav, index) => (
                        <Stack key={index}>
                            <NavLink to={`/course/assigned/offered-course/${courseName}/${offeredCourseId}${nav.link}`} className={` flex flex-row gap-6 w-full text-[14px] items-center md:gap-6 hover:text-paleRed !text-slate-600`}>
                                <Button size="small" variant={location.pathname.startsWith(`/course/assigned/offered-course/${courseName}/${offeredCourseId}${nav.link}`) ? 'contained':'outlined'} className="!flex !justify-start w-full">
                                    {nav.title}
                                </Button>
                            </NavLink>
                        </Stack>
